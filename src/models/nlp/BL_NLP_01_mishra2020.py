...
def run_bl_nlp_01(train_texts: List[str], train_labels: List[str], test_texts: List[str], test_labels: List[str], outdir: str, seed: int, config: Dict) -> Dict:
    ...
    meta["assumptions"].append("URL behavioral modules are stubbed (URLFilterStub).")
    ...
    results = train_and_evaluate_from_splits(train_pre, train_labels, test_pre, test_labels, outdir, seed=seed, cv=cv_param, vectorizer=vec, alpha=alpha)
